...
class ApplicationChrome
{
    ...
    **/
                ...
                using (var activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity")) {
                    using (var window = activity.Call<AndroidJavaObject>("getWindow")) {
                        using (var view = window.Call<AndroidJavaObject>("getDecorView")) {
                            ...
                            view.Call("setSystemUiVisibility", 0);
                            view.Call("setSystemUiVisibility", systemUiVisibilityValue);
                        }
                    }
                }
    ...
    public static void ApplyThemeBars(bool darkMode)
{
    #if USE_ANDROID
        // STATUS BAR
        statusbar_light_mode = !darkMode;

        // NAV BAR COLORS
        statusBarColor = darkMode ? 0xFF000000 : 0xFFFFFFFF;
        navigationBarColor = darkMode ? 0xFF000000 : 0xFFFFFFFF;

        // STATES (wichtig stabil)
        statusBarState = States.VisibleOverContent;
        navigationBarState = States.Visible;
