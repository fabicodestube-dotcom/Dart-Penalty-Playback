...
public class ATCPlayerListItem : MonoBehaviour
{
    ...
    public GameObject streakIndicator; // Optional: Ein visuelles Element, das anzeigt, ob der Spieler in einer Treffer-Serie ist
    ...
    public void Bind(ATCUIStats stats)
    {
        ...
        if (stats.hitsInRound >= 3)
        {
            if (streakIndicator != null)
                streakIndicator.SetActive(true);
        }
        else
        {
            if (streakIndicator != null)
                streakIndicator.SetActive(false);
        }
